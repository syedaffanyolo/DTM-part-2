//
//  dtmApi.swift
//  DTM
//
//  Created by Syedaffan on 10/21/20.
//

import Foundation




struct grNoida : Codable{
    
    let gvname : String
    let gvshops:[String]
    let gvmallimage:String
    let gvshopimages:[String]
    let gvid:Int
    let gvshopnames:[String]
    let gvshopnumbers:[Int]
    let gvshopdiscriptions:[String]
    
    let mmname : String
    let mmshops:[String]
    let mmmallimage:String
    let mmshopimages:[String]
    let mmid:Int
    let mmshopnames:[String]
    let mmshopnumbers:[Int]
    let mmshopdiscriptions:[String]
    
    let apname : String
    let apshops:[String]
    let apmallimage:String
    let apshopimages:[String]
    let apid:Int
    let apshopnames:[String]
    let apshopnumbers:[Int]
    let apshopdiscriptions:[String]
    
    let oaname : String
    let oashops:[String]
    let oamallimage:String
    let oashopimages:[String]
    let oaid:Int
    let oashopnames:[String]
    let oashopnumbers:[Int]
    let oashopdiscriptions:[String]
    
    let dmname : String
    let dmshops:[String]
    let dmmallimage:String
    let dmshopimages:[String]
    let dmid:Int
    let dmshopnames:[String]
    let dmshopnumbers:[Int]
    let dmshopdiscriptions:[String]
    
    let lcname : String
    let lcshops:[String]
    let lcmallimage:String
    let lcshopimages:[String]
    let lcid:Int
    let lcshopnames:[String]
    let lcshopnumbers:[Int]
    let lcshopdiscriptions:[String]
    
    let ggname : String
    let ggshops:[String]
    let ggmallimage:String
    let ggshopimages:[String]
    let ggid:Int
    let ggshopnames:[String]
    let ggshopnumbers:[Int]
    let ggshopdiscriptions:[String]
    
    let gpname : String
    let gpshops:[String]
    let gpmallimage:String
    let gpshopimages:[String]
    let gpid:Int
    let gpshopnames:[String]
    let gpshopnumbers:[Int]
    let gpshopdiscriptions:[String]
    
    let vsname : String
    let vsshops:[String]
    let vsmallimage:String
    let vsshopimages:[String]
    let vsid:Int
    let vsshopnames:[String]
    let vsshopnumbers:[Int]
    let vsshopdiscriptions:[String]
    
    let tcname : String
    let tcshops:[String]
    let tcmallimage:String
    let tcshopimages:[String]
    let tcid:Int
    let tcshopnames:[String]
    let tcshopnumbers:[Int]
    let tcshopdiscriptions:[String]
    
    let ccname : String
    let ccshops:[String]
    let ccmallimage:String
    let ccshopimages:[String]
    let ccid:Int
    let ccshopnames:[String]
    let ccshopnumbers:[Int]
    let ccshopdiscriptions:[String]
    
    let cmname : String
    let cmshops:[String]
    let cmmallimage:String
    let cmshopimages:[String]
    let cmid:Int
    let cmshopnames:[String]
    let cmshopnumbers:[Int]
    let cmshopdiscriptions:[String]
    
    let dsname : String
    let dsshops:[String]
    let dsmallimage:String
    let dsshopimages:[String]
    let dsid:Int
    let dsshopnames:[String]
    let dsshopnumbers:[Int]
    let dsshopdiscriptions:[String]
    
    let amname : String
    let amshops:[String]
    let ammallimage:String
    let amshopimages:[String]
    let amid:Int
    let amshopnames:[String]
    let amshopnumbers:[Int]
    let amshopdiscriptions:[String]
    
    let pmname : String
    let pmshops:[String]
    let pmmallimage:String
    let pmshopimages:[String]
    let pmid:Int
    let pmshopnames:[String]
    let pmshopnumbers:[Int]
    let pmshopdiscriptions:[String]
    
    let scname : String
    let scshops:[String]
    let scmallimage:String
    let scshopimages:[String]
    let scid:Int
    let scshopnames:[String]
    let scshopnumbers:[Int]
    let scshopdiscriptions:[String]
    
}


